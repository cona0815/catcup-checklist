...
- 正式 GAS 是 v1.1 學生帳號版；v1.6 新增獨立 `WorkBoard.gs`，只在現有 `doPost` 加 `getWorkData`、`saveWorkBoard`、`saveFeatureConfig` 三個路由。伙伴清單必須驗證同隊學生或教師，**不可開放 GET 以隊名直接查詢**。
- 新資料分頁 `work_boards`（各隊各組的順序、分工）與 `feature_config`（教師設定）由新端點首次使用時建立。舊資料分頁不改欄位。
- 必做功能 ID（A01–A12、G01–G14）與 Scratch 原有課程 ID 保持不變，避免既有進度對錯項目。
