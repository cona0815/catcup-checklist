...
const saveConfig=context.saveFeatureConfig_({group:'anim',config:{
  added:['A16'],hidden:['A01'],order:['A12','A16','A01'],
  items:{A16:{name:'新功能',cond:'完成一個作品',pri:2,links:[{title:'教學',url:'https://example.com'}]}}
}},'teacher-test');
assert.equal(saveConfig.ok,true);
assert.equal(saveConfig.data.config.added[0],'A16');
assert.equal(saveConfig.data.config.items.A16.links[0].title,'教學');
...
let result=context.saveWorkBoard_({team:'TeamA',group:'anim',credential,
  patch:{assignments:{A16:'50102'},notes:{A16:'我先做角色'},order:['A16','A12']}},'');
