...
(async()=>{
  ...
  try{
    ...
    await page.locator('[data-feature-name=A13]').waitFor();
    assert(config.added.includes('A13'));
    await page.locator('[data-feature-link-title=A13]').fill('參考網站');
    await page.locator('[data-feature-link-url=A13]').fill('https://example.com/lesson');
    await page.locator('[data-feature-link-add=A13]').click();
    assert.equal(config.items.A13.links[0].title,'參考網站');
    page.on('dialog',dialog=>dialog.accept());
    await page.locator('[data-feature-delete=A13]').click();
    assert(config.hidden.includes('A13'));
    await page.locator('[data-feature-restore=A13]').click();
    assert(!config.hidden.includes('A13'));
    ...
    await sp.locator('[data-assign=A13]').waitFor();
    assert.equal(await sp.locator('[data-work-move]').count(),0);
    assert.equal(await sp.locator('[data-feature-add]:visible').count(),0);
    await sp.locator('[data-assign=A13]').selectOption('50102');
    await sp.locator('[data-work-note=A13]').fill('我先做角色');
    await sp.locator('[data-work-note=A13]').blur();
    assert(patches.some(p=>p.assignments?.A13==='50102'));
    assert(patches.some(p=>p.notes?.A13==='我先做角色'));
